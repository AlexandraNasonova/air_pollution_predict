        ...
        for lag in lags:
            for preagg in preagg_methods:
                ...
                for alpha in ewm_params.get(filter_col, []):
                    ...
                    new_names = {x: "{0}_lag{1}d_ewm{2}_{3}{4}{5}".\
                        format(x, lag, alpha, key_str, preagg_str, filter_col_str)
                ...
                for w in windows.get(filter_col, []):
                    for method in agg_methods:
                        ...
                        new_names = {x: "{0}_lag{1}d_win{2}_{3}{4}ag:{5}_{6}".\
                                     format(x, lag, w, key_str, preagg_str, method_name, filter_col_str)
                                     for x in target_cols}
