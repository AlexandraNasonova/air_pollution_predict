        ...
        for csv_url in csv_list:
            ...
            for i in range(RELOAD_COUNTER + 1):
                if i == RELOAD_COUNTER:
                    ...
                    log_error(save_path=sub_path, text=f"Reload attempt {repeated_reloads_for_dif_files}", url=csv_url)
                    break
                try:
                    csv_file = await load_csv_file(csv_url)
                    await save_csv_file_from_str(save_path=save_path, file_name=file_name, csv_file=csv_file)
                    break
...
async def load_csv_file(url) -> str:
